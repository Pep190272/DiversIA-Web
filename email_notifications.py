...
def save_notification_backup(notification_type, data, email_content=None):
    """
    Sistema de respaldo - guardar notificación en base de datos cuando falle el email
    """
    try:
        from app import db
        ...
        notification = NotificationBackup(
            ...
            destinatario=data.get('email', 'diversiaeternals@gmail.com'),
            asunto=f"VERIFICAR: Nueva Asociación - {data.get('nombre_asociacion', 'Sin nombre')}",
            contenido=str(data),
        ...
        print(f"✅ Notificación guardada como respaldo (ID: {notification.id})")
        return notification.id
        
    except Exception as e:
        print(f"❌ Error guardando notificación de respaldo: {e}")
        return None
...
def send_association_registration_notification(association_data):
    ...
    Sistema con respaldo automático si falla SendGrid
    """
    sendgrid_key = os.environ.get('SENDGRID_API_KEY')
    ...
    # Intentar enviar por SendGrid primero
    email_sent = False
    
    if not sendgrid_key:
        print("⚠️ SENDGRID_API_KEY no configurada - usando sistema de respaldo")
    else:
        try:
            sg = SendGridAPIClient(sendgrid_key)
            ...
            if response.status_code == 202:
                print(f"✅ Notificación enviada por email a DiversIA: {response.status_code}")
                email_sent = True
            else:
                print(f"⚠️ Respuesta inesperada de SendGrid: {response.status_code}")
                
        except Exception as e:
            print(f"❌ Error enviando por SendGrid: {str(e)}")
    
    # Sistema de respaldo - SIEMPRE guardar la notificación
    backup_id = save_notification_backup(
        'asociacion_registro', 
        association_data, 
        diversia_email_content
    )
    
    # Si el email falló pero se guardó el respaldo, aún es un éxito parcial
    if not email_sent and backup_id:
        print(f"📋 Notificación guardada como respaldo - ID: {backup_id}")
        print(f"🔍 Accede a /admin/verificar-asociaciones para revisar solicitudes pendientes")
        return True
    elif email_sent:
        return True
    else:
        print(f"❌ Falló tanto el email como el sistema de respaldo")
        return False
